...
type EntityResponseType = HttpResponse<ITournamentTennis>;
type EntityArrayResponseType = HttpResponse<ITournamentTennis[]>;

@Injectable({ providedIn: 'root' })
export class TournamentTennisService {
  public resourceUrl = SERVER_API_URL + 'api/tournaments';

  constructor(protected http: HttpClient) {}

  create(tournament: ITournamentTennis): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(tournament);
    return this.http
      .post<ITournamentTennis>(this.resourceUrl, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }

  update(tournament: ITournamentTennis): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(tournament);
    return this.http
      .put<ITournamentTennis>(this.resourceUrl, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<ITournamentTennis>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<ITournamentTennis[]>(this.resourceUrl, { params: options, observe: 'response' })
