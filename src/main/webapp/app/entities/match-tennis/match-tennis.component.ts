...
@Component({
  selector: 'jhi-match-tennis',
  templateUrl: './match-tennis.component.html'
})
export class MatchTennisComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.matchService
      ...
      .subscribe(
        (res: HttpResponse<IMatchTennis[]>) => this.paginateMatches(res.body, res.headers),
  ...
  trackId(index: number, item: IMatchTennis) {
    return item.id;
  }
  ...
  protected paginateMatches(data: IMatchTennis[], headers: HttpHeaders) {
